...
try:
    while True:
        try:
            ...
            if history.count("weed") >= CONFIRM_FRAMES:
                if last_state != "action":
                    ...
                    cv2.putText(frame, "STATUS: CUTTING!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 3)
                    cv2.imwrite("/home/yash/crop_weed_bot/evidence.jpg", frame)
                    print(">>> Target confirmed! Saved annotated photo to evidence.jpg <<<")
                ...
                # 2. ONLY send motor commands if we are currently stopped
